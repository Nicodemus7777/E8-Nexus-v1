...
import { Send, Sparkles, Loader2, User, Bot, FlaskConical, X, Zap, Spline, Share2, Telescope, RefreshCw } from 'lucide-react';
...
const ChatAssistant: React.FC<ChatAssistantProps> = ({ simulationQueue, removeFromQueue, clearQueue, theme, isBreakingLabActive, activeDefect, entangledPair, ripplePulse, currentMatrix }) => {
  ...
  const lastExplainedDefect = useRef<string | null>(null);
  ...
  // Debounced Effect for Topological Defects
  useEffect(() => {
    if (activeDefect && isBreakingLabActive) {
      const defectId = `${activeDefect.type}-${activeDefect.strength}`;
      if (lastExplainedDefect.current === defectId) return;
      
      const timer = setTimeout(async () => {
        setIsLoading(true);
        lastExplainedDefect.current = defectId;
        const prompt = `A ${activeDefect.type} topological defect has been injected into the E8 lattice with a strength of ${activeDefect.strength}. Explain how this breaks the E8 symmetry specifically in this context.`;
        const response = await askE8Expert([...messages, { role: 'user', content: prompt }]);
        setMessages(prev => [...prev, { role: 'model', content: response }]);
        setIsLoading(false);
      }, 1000); // 1s debounce to avoid spamming during slider movements
      
      return () => clearTimeout(timer);
    }
  }, [activeDefect, isBreakingLabActive]);
  ...
  return (
    <div className={`flex flex-col h-full transition-colors duration-500 ${panelBg}`}>
      <div className={`p-4 border-b flex items-center justify-between ${theme === 'primal' ? 'border-slate-800' : 'border-slate-100'}`}>
        ...
        <button 
          onClick={() => setMessages([{ role: 'model', content: "Memory cleared. Nexus observers ready for new inquiries." }])}
      ...
      {simulationQueue.length > 0 && (
        <div className={`p-3 border-t animate-in slide-in-from-bottom duration-300 ${stageBg}`}>
          <div className="flex justify-between items-center mb-2 px-1">
            <span className="text-[10px] font-mono text-slate-500 uppercase">Queue: {simulationQueue.length} roots</span>
            <button onClick={clearQueue} className="text-[9px] text-red-500 hover:text-red-400 uppercase font-bold">Clear</button>
          </div>
          <button 
            onClick={runSimulation}
            disabled={isLoading || simulationQueue.length < 1}
      ...
      <div className={`p-4 transition-colors ${theme === 'primal' ? 'bg-slate-950/50' : 'bg-slate-50'}`}>
        <div className="relative">
          ...
          <button
            ...
            className="absolute right-1.5 top-1.5 p-1.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white rounded-full transition-colors"
          >
            <Send size={16} />
          </button>
        </div>
